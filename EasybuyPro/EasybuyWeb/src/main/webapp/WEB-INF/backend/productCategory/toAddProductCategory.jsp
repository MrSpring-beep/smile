...
<table border="0" class="add_tab" style="width:930px;" cellspacing="0" cellpadding="0">
    <tr>
        ...
        <tr  <c:if test="${productCategory.type==1}">style="display:none"</c:if> id="tr2">
        ...
        <tr <c:if test="${productCategory.type!=3}">style="display:none"</c:if> id="tr3">
            ...
            <td>
                <select class="jj" name="productCategoryLevel2" style="background-color:#f6f6f6;"
                    ...
                    <option value="0" selected="selected">请选择...</option>
                    <c:forEach items="${productCategoryLevel2}" var="temp">
                        <option value="${temp.id}"
...
<p align="right">
    ...
    <c:if test="${productCategory.id!=null}">
        <a href="javascript:void(0);" onclick="updateOrUpdate();" class="add_b">确认修改</a>
    </c:if>
    <c:if test="${productCategory.id==null}">
        <a href="javascript:void(0);" onclick="saveOrUpdate();" class="add_b">确认新增</a>
    </c:if>
</p>
</div>